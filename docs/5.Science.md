...
# B. Photometry Pipeline for Learning (Spanish) | LCO Data

[GitHub Respostory - Photometry BVO Notebook](https://github.com/GabrielJaimes-AstroDev/Bolivian-Virtual-Observatory-BVO-/blob/main/1.Photometry_BVO.ipynb)
...
``` python
    ...
    sigma = 3  # Standard deviation (sigma) used in image processing and star detection algorithms
    threshold = 100  # Detection threshold for source identification in astronomical images
    center_threshold = 30  # Detection threshold for identifying the center of objects in an image
...
from photutils.aperture import CircularAperture, CircularAnnulus, aperture_photometry
    ...
    # Extract WCS from the FITS header
    wcs = WCS(header)

    # Convert RA and Dec to pixel coordinates
    coord = SkyCoord(ob_ra * u.deg, ob_de * u.deg)
    pixel_coords = wcs.world_to_pixel(coord)

    # Calculate background statistics
    background_mean = np.mean(data)
    background_median = np.median(data)
    background_std = np.std(data)

    # Perform DAOFind to detect stars in the image
    bkg_sigma = mad_std(data)  # Background noise estimation
    daofind = DAOStarFinder(fwhm=3.0, threshold=5.*bkg_sigma)
    sources = daofind(data - background_median)

    # Filter sources based on distance from specified RA, Dec
    distances = np.sqrt((sources['xcentroid'] - pixel_coords[0])**2 + (sources['ycentroid'] - pixel_coords[1])**2)
    selected_sources = sources[distances < center_threshold]  # Select stars within threshold distance
    ...
    # Visualize apertures over the image
    norm = ImageNormalize(vmin=vmin, vmax=vmax, stretch=SqrtStretch())
    plt.figure(figsize=(30, 15))
    ...
    annulus_apertures.plot(color='blue', lw=1.5, alpha=0.7)

    # Add legend to distinguish aperture types
    plt.legend(["Circular Apertures", "Annulus Apertures"])
